package gamer

import Desc
import Node
import fold
import moveAll
import java.lang.IllegalArgumentException

class DeepGamer(
    private val finishDesc: Desc,
    private val maxDeep: Int,
    private val visitor: (Desc) -> Unit
) : (Desc) -> Desc {

    override fun invoke(desc: Desc): Desc {
        val head = Node(desc)
        return next(head, 0, head, mutableListOf())?.value ?: throw IllegalArgumentException()
    }

    private fun next(head: Node<Desc>, deep: Int, prevNode: Node<Desc>, listHash: MutableList<Int>): Node<Desc>? {
        visitor(prevNode.value)



        if (prevNode.value == finishDesc) return prevNode
        if (deep == maxDeep) {
            return null
        }
        listHash.add(prevNode.value.hashCode())

        val descs = prevNode.value.moveAll()

        val newDescs = descs.filter { !listHash.contains(it.hashCode()) }

        prevNode.nextNodes = newDescs.map { Node(it) }
        prevNode.nextNodes.forEach {
            val next = next(head, deep + 1, it, listHash)
            if (next != null) return next
        }
        return null
    }
}